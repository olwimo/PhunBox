...
public class CoreMidiDeviceProvider extends MidiDeviceProvider implements CoreMidiNotification {
    ...
    private static final class MidiProperties {
        ...
        private int midiClientReference;
    ...
    private synchronized void initialise() throws CoreMidiException {
        ...
        if ( midiProperties.client == null ) {
            ...
            midiProperties.client = new CoreMidiClient("Core MIDI Provider", ref -> {
                midiProperties.midiClientReference = ref;
            });
    ...
    /**
     * Creates a virtual CoreMidiSource
     *
     * @param sourceName The name of the virtual source
     *
     */

    public CoreMidiVirtualSource createSource(String sourceName) throws CoreMidiException {

        // Holder<CoreMidiDeviceInfo> holder = new Holder<>();
        return new CoreMidiVirtualSource(this, this.createSource(midiProperties.midiClientReference, sourceName));

    }

    /**
     * Creates a virtual CoreMidiDestination
     *
     * @param destinationName The name of the virtual destination
     *
     */

    public CoreMidiVirtualDestination createDestination(String destinationName) throws CoreMidiException {
        return new CoreMidiVirtualDestination(this,
                midiProperties.midiClientReference, destinationName);
    }

/*    CoreMidiDeviceInfo getCatchingInfo(int ref, Consumer<Co>)

    @FunctionalInterface
    public interface ThrowingInfoGetter {
        CoreMidiDeviceInfo get(Integer ref) throws CoreMidiException;
    }
*/
/*    static Function<Integer, CoreMidiDeviceInfo> throwingWrapper(ThrowingInfoGetter throwingGetter) {

        return i -> {
            try {
                return throwingGetter.get(i);
...
/*
    static Function<Integer, CoreMidiDeviceInfo> handlingWrapper(ThrowingInfoGetter throwingGetter) {
        return i -> {
            try {
                return throwingGetter.get(i);
            } catch (Exception e) {
                try {
                    CoreMidiException exCast = CoreMidiException.class.cast(e);
                    // e.printStackTrace();
...
*/
        ...
        if ( listener != null ) {
            ...
            if (listener instanceof CoreMidiDeviceProvider) {
                ...
                if (!isLibraryLoaded() && changeScanner.get() == null) {
                    ...
                    }, "Midnekt Environment Change Scanner");
    ...
    public static String getLibraryVersion() {
        ...
        return Package.getPackage("io.github.olwimo.midi").getImplementationVersion();
    ...
    protected native CoreMidiDeviceInfo getMidiDeviceInfo(int reference) throws CoreMidiException;
    ...
    protected native long getMicroSecondTime();

    /**
     * Creates a CoreMIDI Destination Callback
     *
     * @param callback 				The destination callback
     *
     * @return								A reference to the created callback
     *
     * @throws 								CoreMidiException if the callback cannot be created
     *
     */

    protected native long createCallback(CoreMidiVirtualDestination callback) throws CoreMidiException;

    /**
     * Creates a CoreMIDI destination
     *
     * @param clientReference	The MIDI client reference
     * @param callback 				The callback destination
     * @param destinationName 				The name of the destination
     *
     * @return								A reference to the created destination
     *
     * @throws 								CoreMidiException if the destination cannot be created
     *
     */

    protected native int createDestination(int clientReference, long callback, String destinationName) throws CoreMidiException;

    /**
     * Creates a CoreMIDI source
     *
     * @param clientReference	The MIDI client reference
     * @param sourceName 				The name of the source
     *
     * @return								A reference to the created source
     *
     * @throws 								CoreMidiException if the source cannot be created
     *
     */

    private native int createSource(int clientReference, String sourceName) throws CoreMidiException;

    /**
     * Distributes incoming MIDI from a source to the client input ports which are connected to that source
     *
     * @param sourceEndPointReference   The device to send the message from
    ...
    protected native void midiReceived(int sourceEndPointReference, MidiMessage message, long timestamp) throws CoreMidiException;
