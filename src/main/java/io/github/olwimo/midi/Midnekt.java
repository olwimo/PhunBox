...
public class Midnekt extends Thread {
    enum Mode {
        Design(new MidnektMode.DesignMode()),
        Instrument(new MidnektMode.InstrumentMode()),
        Device(new MidnektMode.DeviceMode()),
        Mixer(new MidnektMode.MixerMode());
        MidnektMode self;

        Mode(MidnektMode self) {
            this.self = self;
        }
    }
    class MidnektReceiver extends Thread implements Receiver {
        Midnekt midnekt;
        MidnektReceiver(Midnekt midnekt) { this.midnekt = midnekt; }
...
/*        for (Mode mode : Mode.values()) {
    ...
    long  startTime;
    Midnekt() throws MidiUnavailableException {
/*        if (main != null)
            main.close();
        main = this;*/
//            this.uuid = UUID.randomUUID();
    ...
    @Override
    public void run() {
        CoreMidiVirtualSource out = null;
        CoreMidiVirtualDestination in = null;
        MidiDevice lpOut = null;
        // CoreMidiReceiver rout = null;
        try {
            System.out.println("Working MIDI Devices:");
            // CoreMidiDeviceProvider myMidiDeviceProvider = new CoreMidiDeviceProvider();
            CoreMidiDeviceProvider coreMidiDeviceProvider = new CoreMidiDeviceProvider();

            CoreMidiDeviceProvider.getMIDIClient().inputPortCreate("Midnekt");

            MidiDevice.Info[] info = CoreMidiDeviceProvider.getMidiDeviceInfo();
            for (MidiDevice.Info device : info) {
                System.out.println("  " + device);
            }

            if (Midnekt.isCoreMidiLoaded()) {
                System.out.println("Midnekt native library is running.");
            } else {
                System.out.println("Midnekt native library is not available.");
            }

            // CoreMidiClient client = new CoreMidiClient("Midnekt");

            List<MidiDevice.Info> launchpads = Arrays.stream(info)
                    .filter(i -> i.getDescription().compareTo("Launchpad S") == 0).collect(Collectors.toList());

            for (MidiDevice.Info launchpad : launchpads) {
                device = coreMidiDeviceProvider.getDevice(launchpad);
                if (device.getMaxTransmitters() == -1 || device.getMaxTransmitters() > 0)
                    break;
            }

            for (MidiDevice.Info launchpad : launchpads) {
                lpOut = coreMidiDeviceProvider.getDevice(launchpad);
                if (lpOut.getMaxReceivers() == -1 || device.getMaxReceivers() > 0)
                    break;
            }

            out = coreMidiDeviceProvider.createSource("Midnekt");
            in = coreMidiDeviceProvider.createDestination("Midnekt");
            // rout = out.getTransmitter();
...
//                .matches("(Launchpad|Launchpad S|Launchpad Mini)")).findFirst().get());
            //Transmitter lpTrans = device.getTransmitter();
...
//        sequencer.addControllerEventListener(this.new MidnektListener(), controllers);
        ...
        } catch (MidiUnavailableException | CoreMidiException e) {
            e.printStackTrace();
        }
...
//        super.run();
        ...
        while (!scanner.nextLine().matches("x.*")) {
            try {
                out.midiReceived(new ShortMessage(ShortMessage.START));
            } catch (InvalidMidiDataException | CoreMidiException e) {
                e.printStackTrace();
            }
        }
    }
        ...
        return Stream.concat(Stream.of(new Tuple<Integer, Mode>(108, Mode.Design),
                ...
                new Tuple<Integer, Mode>(111, Mode.Mixer))
                        .map(im -> new Tuple<Predicate<Tuple<MidiMessage, Long>>, Consumer<Tuple<MidiMessage, Long>>>
                                (msg -> {
                                    // TODO: Not all messages are Short!
...
/*        Function<MidnektStarter, Midnekt> make;
    ...
    static final class DesignMode extends MidnektMode {
/*        MidnektStarter starter;
    ...
    static final class InstrumentMode extends MidnektMode {
/*        MidnektStarter starter;
    ...
    static final class DeviceMode extends MidnektMode {
/*        MidnektStarter starter;
    ...
    static final class MixerMode extends MidnektMode {
/*        MidnektStarter starter;
